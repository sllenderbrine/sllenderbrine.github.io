    ...
    static async fromUrls(args: [name:string, url:string][], padding = 0) {
        ...
        for(let [name, url] of args) {
            promises.push(new Promise<void>(async res => {
                let img = new Image();
                img.onload = () => {
                    let data: TextureAtlasImage = {img, x:0, y:0, w:img.naturalWidth+2*padding, h:img.naturalHeight+2*padding, name};
        ...
        let bounds: {[name:string]: TextureAtlasImage} = {};
        ...
        const TextureAtlasImage = await new Promise<HTMLImageElement>(res => {
            let img = new Image();
            img.onload = () => {
                res(img);
            }
